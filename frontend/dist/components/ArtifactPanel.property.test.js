...
describe('ArtifactPanel - Property Tests', () => {
    ...
    it('Property 8: renders all structured output fields', () => {
        const speakingStyleArb = fc.constantFrom('soft', 'flirty', 'serious');
        ...
        fc.assert(fc.property(noteDraftArb, tagsArb, speakingStyleArb, (noteDraft, tags, toneLabel) => {
            const onRefinement = vi.fn();
            const onCopy = vi.fn();
            const { unmount } = render(_jsx(ArtifactPanel, { noteDraft: noteDraft, tags: tags, toneLabel: toneLabel, onRefinement: onRefinement, onCopy: onCopy }));
            try {
    ...
    it('Property 9: refinement buttons map to correct request types', () => {
        ...
        const buttonTextArb = fc.constantFrom('Make it shorter', 'Make it bolder', 'Make it more romantic', 'Translate to French');
        fc.assert(fc.property(buttonTextArb, (buttonText) => {
            const onRefinement = vi.fn();
            const onCopy = vi.fn();
            const { unmount } = render(_jsx(ArtifactPanel, { noteDraft: "Sample note for testing", tags: ['tag1'], toneLabel: "soft", onRefinement: onRefinement, onCopy: onCopy }));
            try {
...
//# sourceMappingURL=ArtifactPanel.property.test.js.map