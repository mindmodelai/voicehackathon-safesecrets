...
describe('Avatar State Machine - Property Tests', () => {
    ...
    it('Property 1: maintains priority order (listening > speaking > thinking > idle)', () => {
        ...
        const avatarEventArb = fc.oneof(fc.constant({ type: 'USER_SPEAKING_START' }), fc.constant({ type: 'USER_SPEAKING_END' }), fc.record({
            type: fc.constant('TTS_START'),
            style: speakingStyleArb,
        }), fc.constant({ type: 'TTS_END' }), fc.constant({ type: 'THINKING_START' }), fc.constant({ type: 'THINKING_END' }));
        fc.assert(fc.property(fc.array(avatarEventArb, { minLength: 1, maxLength: 20 }), (events) => {
    ...
    it('Property 2: video source matches state and style', () => {
        const speakingStyleArb = fc.constantFrom('soft', 'flirty', 'serious');
        fc.assert(fc.property(speakingStyleArb, (style) => {
            const sm = createAvatarStateMachine();
    ...
    it('Additional: state machine is deterministic', () => {
        ...
        fc.assert(fc.property(fc.array(avatarEventArb, { minLength: 1, maxLength: 15 }), (events) => {
...
//# sourceMappingURL=avatar-state-machine.property.test.js.map