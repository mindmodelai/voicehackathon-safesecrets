...
export interface ClosedTunerValues {
  perspective: number;
  originY: number;
  rotateY: number;
  height: number;
...
export interface HeaderProps {
  ...
  closedTuner?: ClosedTunerValues;
  onClosedTunerChange?: (v: ClosedTunerValues) => void;
  tunerText?: string;
  onTunerTextChange?: (v: string) => void;
}

export function Header({ onAboutClick, onTunerToggle, showTuner, noteTuner, onNoteTunerChange, closedTuner, onClosedTunerChange, tunerText, onTunerTextChange }: HeaderProps) {
  const t = noteTuner;
  const set = (key: keyof NoteTunerValues, val: number) => {
    if (t && onNoteTunerChange) onNoteTunerChange({ ...t, [key]: val });
  };
  const ct = closedTuner;
  const setClosed = (key: keyof ClosedTunerValues, val: number) => {
    if (ct && onClosedTunerChange) onClosedTunerChange({ ...ct, [key]: val });
  };
    ...
    <header className={styles.header}>
      ...
      <nav>
        <ul className={styles.nav}>
          ...
          <li><button className={styles.navButton} onClick={onTunerToggle} type="button" style={{ opacity: 0, pointerEvents: 'auto' }}>⚙️</button></li>
        </ul>
      </nav>
      ...
      {showTuner && t && (
        <div style={{ position: 'fixed', top: '60px', right: '10px', zIndex: 9999, background: '#fff', border: '1px solid #ccc', borderRadius: '10px', padding: '14px', fontSize: '0.75rem', boxShadow: '0 4px 20px rgba(0,0,0,0.15)', width: '280px', maxHeight: '90vh', overflowY: 'auto' }}>
          ...
          {ct && (
            <>
              <div style={{ fontWeight: 700, marginTop: '12px', marginBottom: '8px', borderTop: '1px solid #ddd', paddingTop: '10px' }}>Closed State Tuner</div>
              {([
                ['perspective', 100, 1200, 10],
                ['originY', 0, 100, 1],
                ['rotateY', -30, 30, 0.5],
                ...
                ['marginTop', -60, 60, 1],
                ['marginLeft', -60, 60, 1],
              ] as [keyof ClosedTunerValues, number, number, number][]).map(([key, min, max, step]) => (
                <label key={`closed-${key}`} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                  <span style={{ minWidth: '75px' }}>{key}</span>
                  <input type="range" min={min} max={max} step={step} value={ct[key]} onChange={e => setClosed(key, parseFloat(e.target.value))} style={{ flex: 1 }} />
                  <span style={{ minWidth: '40px', textAlign: 'right' }}>{ct[key]}</span>
                </label>
              ))}
            </>
          )}
