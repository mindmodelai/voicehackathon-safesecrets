...
export interface WSServerOptions {
  ...
  /** Optional rate limiter instance (useful for testing). Defaults to 20/min. */
...
export class SafeSecretsWSServer {
  ...
  constructor(options: WSServerOptions = {}) {
    ...
    this.rateLimiter = options.rateLimiter ?? new RateLimiter(20, 60 * 1000);
  ...
  private handleConnection(ws: WebSocket, req: IncomingMessage): void {
    ...
    if (!this.rateLimiter.check(ip)) {
      console.warn(`[WSServer] Connection rejected: Rate limit exceeded for IP ${ip}`);
