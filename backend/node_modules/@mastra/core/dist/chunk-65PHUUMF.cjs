...
var CoreToolBuilder = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  constructor(input) {
    ...
    if (!chunk7UWHFWST_cjs.isVercelTool(this.originalTool) && (input.autoResumeSuspendedTools || this.originalTool.id?.startsWith("agent-"))) {
  ...
  getParameters = () => {
    if (chunk7UWHFWST_cjs.isVercelTool(this.originalTool)) {
  ...
  createExecute(tool, options, logType, processedSchema) {
    ...
    const execFunction = async (args, execOptions) => {
      ...
      try {
        ...
        if (chunk7UWHFWST_cjs.isVercelTool(tool)) {
          result = await chunkCYUP7QWT_cjs.executeWithContext({
            span: toolSpan,
            fn: async () => tool?.execute?.(args, execOptions)
          });
        } else {
          const wrappedMastra = options.mastra ? chunkCYUP7QWT_cjs.wrapMastra(options.mastra, { currentSpan: toolSpan }) : options.mastra;
          ...
          const baseContext = {
            ...
            runId: options.runId,
            requestContext: options.requestContext ?? new chunkVDKWYUGC_cjs.RequestContext(),
          ...
          if (resumeData) {
            const resumeValidation = chunk7UWHFWST_cjs.validateToolInput(resumeSchema, resumeData, options.name);
          ...
          result = await chunkCYUP7QWT_cjs.executeWithContext({ span: toolSpan, fn: async () => tool?.execute?.(args, toolContext) });
        }
        if (suspendData) {
          const suspendSchema = this.getSuspendSchema();
          const suspendValidation = chunk7UWHFWST_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunk7UWHFWST_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunk7UWHFWST_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      try {
        ...
        const { data, error: error2 } = chunk7UWHFWST_cjs.validateToolInput(parameters, args, options.name);
        if (error2) {
  ...
  build() {
    ...
    let needsApprovalFn;
    if (chunk7UWHFWST_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunk7UWHFWST_cjs.Tool) && !chunk7UWHFWST_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-65PHUUMF.cjs.map
//# sourceMappingURL=chunk-65PHUUMF.cjs.map